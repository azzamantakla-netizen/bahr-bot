...
export class Texas4WinApi {
  ...
  public async signIn(): Promise<boolean> {
    try {
      console.log("Attempting agent sign in to:", `${this.baseUrl}/global/api/User/signIn`);
      ...
      if (response.data && (response.data.status || response.data.result)) {
        ...
        console.log("Agent signed in successfully:", response.data.result?.message || "OK");
        return true;
      }
      ...
      throw new Error(error?.response?.data?.notification?.[0]?.content || "تعذر تسجيل الدخول في سيرفر الوكالة");
    }
  }
  ...
  private async postAuth<T = any>(endpoint: string, body: any, isRetry = false): Promise<ApiResponse<T>> {
    ...
    try {
      ...
      if ((status === 403 || status === 401) && !isRetry) {
        console.warn(`Encountered ${status}, re-authenticating agent session and retrying...`);
