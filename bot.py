...
# 2. البيانات الخاصة بك المعتمدة والمثبتة المأخوذة من ملفك
...
# 3. إعداد قاعدة البيانات ونظام طابور الأدوار والصالحيات مع ميزة التطهير
def init_db():
    conn = sqlite3.connect("bot_database.db")
    cursor = conn.cursor()
    
    # خطوة التطهير: مسح جدول المستخدمين القديم لإعادة بنائه بالتصميم الجديد ومنع التعارض
...
async def cmd_start_handler(message: types.Message):
    ...
    # خطوة التطهير التلقائي: حذف أي أزرار شاشة قديمة ومعلقة لدى المستخدم بشكل إجباري ومباشر
    force_remove = types.ReplyKeyboardRemove()
    await message.answer("🔄 جاري تحديث واجهة البوت وتطهير الأزرار القديمة...", reply_markup=force_remove)
...
# خادم ويب مصغر لمنع أخطاء الـ Port في Render
