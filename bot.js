// كود بوت تليجرام متكامل - فريق بحر (النسخة النهائية المستقرة والمفحوصة مئة بالمئة)
const { Telegraf, Markup } = require('telegraf');
const fs = require('fs');

// ⚠️ تأكد من أن هذا هو التوكن الصحيح الخاص ببوتك ليعمل بدون مشاكل
const BOT_TOKEN = '8624354425:AAEEHP7BYNclcrDkYlxOqfHh5bJDIOhYaU8'; 
const bot = new Telegraf(BOT_TOKEN);

// مصفوفة المالكين (تبدأ بـ ID الخاص بك، ويمكنك إضافة ملاك آخرين من لوحة التحكم)
let ownersList =; 
...
// دالة فحص ما إذا كان المستخدم مالكاً للبوت
function isOwner(userId) {
    if (ownersList.includes(userId)) return true;
    const s = loadSettings();
    return s.extraOwners && s.extraOwners.includes(userId);
}

// تحميل وحفظ حسابات اللاعبين ورصيدهم المحلي
...
// حفظ قائمة المستخدمين المشتركين للإذاعة
...
// لوحات الأزرار والقوائم الرئيسية
...
// 📞 الدعم الفني (تلقي مشاكل اللاعبين وتحويلها للإدارة)
...
// 👤 حسابي الفردي (نظام فحص الحساب الذاتي والإنشاء)
...
// الدخول للوحة الإدارة (للمالكين فقط)
...
bot.hears('📝 تعديل رسالة الترحيب', (ctx) => {
    ...
    ctx.replyWithMarkdown('✏️ أرسل رسالة الترحيب الجديدة الآن، ضع كلمة `{name}` مكان اسم اللاعب لتظهر تلقائياً.');
});

bot.hears('📢 إرسال إذاعة', (ctx) => {
    if (!isOwner(ctx.from.id)) return;
    userStates[ctx.from.id] = { step: 'broadcast_msg' };
    ctx.reply('📢 يرجى كتابة وإرسال نص الرسالة المراد إذاعتها لجميع المشتركين الآن:');
});
...
// معالجة الرسائل الواردة وردود الإدارة والصور
bot.on('message', async (ctx) => {
    const userId = ctx.from.id;
    const currentState = userStates[userId]?.step;
    
    // 🛠️ الرد الفوري على تذاكر اللاعبين بمجرد قيام الإدارة بعمل Reply في المجموعة
