...
// النصوص مشفرة بيونيكود معياري نظيف تظهر للمستخدم بالعربية مئة بالمئة
const msgs = {
    welcome_msg: "\u0645\u0631\u062d\u0628\u0627\u064b \u0628\u0643 \u0641\u064a \u0639\u0627\u0626\u0644\u062a\u0646\u0627\u060c \u062a\u0641\u0636\u0644 \u0628\u0627\u0644\u0627\u062e\u062a\u064a\u0623\u0631 \u0645\u0646 \u0627\u0644\u0642\u0627\u0626\u0645\u0629 \u0628\u062d\u0633\u0628 \u0627\u0644\u0632\u0631 \u0627\u0644\u0630\u064a \u064a\u0644\u0628\u064a \u0632\u0644\u0628\u0643.",
...
function getMainMenu(userId) { 
    // أزرار عربية مشفرة: شحن الرصيد، طلب سحب، حسابي الفردي، الدعم الفني
...
function getAdminMenu() { 
    return Markup.keyboard([ 
        ['\u062a\u0639\u062f\u064a\u0644 \u0627\u0644\u0631\u0635\u064a\u062f', '\u0627\u0644\u0639\u0648\u062f\u0629 \u0644\u0644\u0642\u0627\u0626\u0645\u0629']
    ]).resize(); 
} 

bot.start((ctx) => { ctx.reply(loadSettings().welcome_msg, getMainMenu(ctx.from.id)); }); 
bot.hears('\u0627\u0644\u0639\u0648\u062f\u0629 \u0644\u0644\u0642\u0627\u0626\u0645\u0629', (ctx) => { ctx.reply('OK', getMainMenu(ctx.from.id)); }); 
bot.hears('\u0627\u0644\u062f\u0639\u0645 \u0627\u0644\u0641\u0646\u064a', (ctx) => { userStates[ctx.from.id] = { step: 'support' }; ctx.reply(msgs.technical_support); }); 

bot.hears('\u0622\u062d\u0633\u0627\u0628\u064a \u0627\u0644\u0641\u0631\u062f\u064a', (ctx) => { getAccountCard(ctx); });
bot.hears('\u062d\u0633\u0627\u0628\u064a \u0627\u0644\u0641\u0631\u062f\u064a', (ctx) => { getAccountCard(ctx); });

function getAccountCard(ctx) {
    const account = getPlayerAccount(ctx.from.id); 
    if (!account) { 
        return ctx.reply('\u062d\u0633\u0627\u0628\u0643 \u063a\u064a\u0631 \u0645\u0633\u062c\u0644. \u0627\u0636\u063a\u0637 \u0644\u0644\u062a\u0633\u062c\u064a\u0644:', Markup.inlineKeyboard([[Markup.button.callback('\u0625\u0646\u0634\u0627\u0621 \u062d\u0633\u0627\u0628 \u062c\u062f\u064a\u062d', 'register_account')]])); 
    } 
    ctx.replyWithMarkdown(`\u0627\u0633\u0645 \u0627\u0644\u0645\u0633\u062a\u062e\u062f\u0645: \`${account.username}\`\n\u0627\u0644\u0631\u0635\u064a\u062f: *${account.balance.toLocaleString()}* \u0644.\u0633`, 
        Markup.inlineKeyboard([[Markup.button.callback('\u274c \u062d\u0630\u0641 \u062d\u0633\u0627\u0628\u064a', 'confirm_delete_acc')]])); 
}

bot.action('confirm_delete_acc', (ctx) => { ctx.answerCbQuery(); ctx.editMessageText('\u0647\u0644 \u0623\u0646\u062a \u0645\u062a\u0621\u0643\u062f\u061f', Markup.inlineKeyboard([[Markup.button.callback('\u0646\u0639\u0645\u0601 \u0627\u062d\u0630\u0641', 'execute_delete_acc')], [Markup.button.callback('\u0625\u0644\u063a\u0627\u0621', 'cancel_delete_acc')]])); });
bot.action('execute_delete_acc', (ctx) => { ctx.answerCbQuery(); deletePlayerAccount(ctx.from.id); ctx.editMessageText('\u062a\u0645 \u0627\u0644\u062d\u0630\u0641.'); });
bot.action('cancel_delete_acc', (ctx) => { ctx.answerCbQuery(); ctx.editMessageText('\u062a\u0645 \u0627\u0644\u0625\u0644\u063a\u0627\u0621.'); });
bot.action('register_account', (ctx) => { ctx.answerCbQuery(); userStates[ctx.from.id] = { step: 'username' }; ctx.reply('\u0623\u0631\u0633\u0644 \u0627\u0633\u0645 \u0627\u0644\u0645\u0633\u062a\u062e\u062f\u0645:'); }); 

bot.hears('\u0644\u0648\u062d\u0629 \u0627\u0644\u0625\u062f\u0627\u0631\u0629', (ctx) => { 
    if (!isOwner(ctx.from.id)) return ctx.reply('❌ Admin only.'); 
    ctx.reply('Admin Panel:', getAdminMenu()); 
}); 

bot.hears('\u062a\u0639\u062f\u064a\u0644 \u0627\u0644\u0631\u0635\u064a\u062f', (ctx) => {
    if (!isOwner(ctx.from.id)) return;
    userStates[ctx.from.id] = { step: 'admin_id' };
    ctx.reply('\u0623\u0631\u0633\u0644 ID \u0627\u0644\u062a\u0644\u063a\u0631\u0627\u0645 \u0644\u0644\u0639\u0645\u064a\u0644:');
});
...
bot.action('w_s', (ctx) => { ctx.answerCbQuery(); userStates[ctx.from.id] = { step: 'w_amt', method: 'Syriatel' }; ctx.reply('\u0623\u062f\u062e\u0644 \u0627\u0644\u0645\u0628\u0644\u063a:'); });
bot.action('w_c', (ctx) => { ctx.answerCbQuery(); userStates[ctx.from.id] = { step: 'w_amt', method: 'Cham' }; ctx.reply('\u0623\u062f\u062e\u0644 \u0627\u0644\u0645\u0628\u0644\u063a:'); });

bot.action('chg_app', (ctx) => { ctx.answerCbQuery(); ctx.reply('OK'); });
